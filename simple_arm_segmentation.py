...
if detection_result.pose_landmarks:
    ...
    # Shoulder: Special handling
    # - Top: Use perpendicular search to find point directly above shoulder
    # - Bottom: Use symmetric distance approach to find armpit point (not strictly perpendicular)
    
    # Get contour for shoulder analysis
    ...
    if contours_for_shoulder:
        ...
        # Calculate perpendicular direction for shoulder
        dx, dy = shoulder_direction
        length = np.sqrt(dx**2 + dy**2)
        perp_x = -dy / length
        perp_y = dx / length
        arm_x = dx / length
        ...
        # Determine which perpendicular direction is "up" (toward top of arm)
        # We want the direction that has negative y component (upward in image coordinates)
        # Check both perpendicular directions and choose the one going upward
        if perp_y > 0:  # This direction goes down, so flip it
            perp_x = -perp_x
            perp_y = -perp_y
        
        # Find TOP point: Search along perpendicular direction (upward)
        shoulder_top = None
        last_inside = None
        
        for dist in range(1, 250):
            test_x = int(shoulder_coords[0] + perp_x * dist)
            test_y = int(shoulder_coords[1] + perp_y * dist)
            ...
            # Track last inside point
            if current_value == 255:
                last_inside = (test_x, test_y)
            
            # Look for edge transition from inside to outside
            if last_inside and current_value == 0:
                shoulder_top = last_inside
                break
        
        # If no edge found by transition, find nearest contour point in perpendicular direction
        if shoulder_top is None:
            candidates_top = []
            for cp in contour_points_shoulder:
                vec = (cp[0] - shoulder_coords[0], cp[1] - shoulder_coords[1])
                perp_proj = vec[0] * perp_x + vec[1] * perp_y
                
                if perp_proj > 5:  # In the top direction
                    arm_proj = abs(vec[0] * arm_x + vec[1] * arm_y)
                    if arm_proj < 30:  # Stricter alignment - must be close to perpendicular
            ...
            if candidates_top:
                # Sort by best alignment (smallest arm_proj = most perpendicular)
                candidates_top.sort(key=lambda c: c['arm_proj'])
                shoulder_top = candidates_top[0]['point']
        
        # Find BOTTOM point: Use symmetric distance approach (armpit, not strictly perpendicular)
        shoulder_bottom = None
        if shoulder_top:
            top_distance = np.sqrt((shoulder_top[0] - shoulder_coords[0])**2 + 
                                  (shoulder_top[1] - shoulder_coords[1])**2)
            
            # Find contour point at similar distance in opposite direction
            candidates_bottom = []
            for cp in contour_points_shoulder:
                vec = (cp[0] - shoulder_coords[0], cp[1] - shoulder_coords[1])
                perp_proj = vec[0] * (-perp_x) + vec[1] * (-perp_y)
                ...
                if perp_proj > 5:  # In the bottom direction
                    ...
                    # More relaxed alignment for armpit (can be off-perpendicular)
                    if distance_diff < top_distance * 0.35 and arm_proj < 60:
                        candidates_bottom.append({
                            'point': tuple(cp),
                            'distance': distance,
                            'distance_diff': distance_diff,
            ...
            if candidates_bottom:
                candidates_bottom.sort(key=lambda c: (c['distance_diff'], c['arm_proj']))
                shoulder_bottom = candidates_bottom[0]['point']
    else:
        shoulder_top = None
        shoulder_bottom = None
    ...
    for landmark_name, data in perpendicular_data.items():
        ...
        if top and bottom:
            ...
            # Only bottom point available
            # Draw thin line from center to bottom
            cv2.line(img_with_overlay, center, bottom, cyan_color, 1)
            
            # Draw small MAGENTA intersection point at bottom
            cv2.circle(img_with_overlay, bottom, 3, magenta_color, -1)
            cv2.circle(img_with_overlay, bottom, 4, (0, 0, 0), 1)
    ...
    # Numbering follows pairwise pattern: each perpendicular has consecutive IDs (top, bottom)
    # ========================================================================
    ...
    # Prepare landmark data for CSV export in pairwise order
    landmark_data = []
    landmark_id = 1
    
    # ===== PAIRWISE NUMBERING: Top and Bottom for each perpendicular =====
    
    # 1-2. Shoulder (top, bottom)
    if shoulder_top:
    ...
    if shoulder_bottom:
        landmark_data.append({
            ...
            'Type': 'Anatomical',
            'Category': 'Main',
            'Name': 'Shoulder_Bottom',
            'X_Pixel': shoulder_bottom[0],
            'Y_Pixel': shoulder_bottom[1],
            'Width_Pixel': 'N/A',
            'Color': 'magenta'
        })
        landmark_id += 1
    
    # 3-4, 5-6, 7-8. Upper arm semi-landmarks (top, bottom pairs)
    ...
    # 11-12, 13-14, 15-16. Forearm semi-landmarks (top, bottom pairs)
    ...
    # ===== MAIN ANATOMICAL CENTERS (for reference) =====
    ...
    def visualize_landmark_linkage(csv_file, output_image='arm_landmark_linkage.jpg'):
        ...
        # Collect all edge points and separate them into actual top (smaller Y) and bottom (larger Y)
        # This ensures no crossing lines
        
        all_landmarks = []
        
        # Collect all landmarks with their positions
        for idx, row in df.iterrows():
            if 'Top' in row['Name'] or 'Bottom' in row['Name']:
        ...
        # Group by perpendicular line (shoulder, semi1, semi2, etc.)
        # For each group, assign to top edge (smaller Y) or bottom edge (larger Y)
        
        top_edge_points = []
        bottom_edge_points = []
        
        # Process shoulder
        shoulder_points = [p for p in all_landmarks if 'Shoulder' in p['name']]
        if len(shoulder_points) == 2:
            shoulder_points.sort(key=lambda p: p['y'])  # Sort by Y
            top_edge_points.append((shoulder_points[0]['x'], shoulder_points[0]['y']))  # Smaller Y = top
            bottom_edge_points.append((shoulder_points[1]['x'], shoulder_points[1]['y']))  # Larger Y = bottom
        
        # Process upper arm semi-landmarks
        for i in range(1, 4):
            semi_points = [p for p in all_landmarks if f'UpperArm_Semi{i}' in p['name']]
            if len(semi_points) == 2:
                semi_points.sort(key=lambda p: p['y'])
                top_edge_points.append((semi_points[0]['x'], semi_points[0]['y']))
                bottom_edge_points.append((semi_points[1]['x'], semi_points[1]['y']))
        
        # Process elbow
        elbow_points = [p for p in all_landmarks if 'Elbow' in p['name'] and p['name'] != 'Elbow_Center']
        if len(elbow_points) == 2:
            elbow_points.sort(key=lambda p: p['y'])
            top_edge_points.append((elbow_points[0]['x'], elbow_points[0]['y']))
            bottom_edge_points.append((elbow_points[1]['x'], elbow_points[1]['y']))
        
        # Process forearm semi-landmarks
        for i in range(1, 4):
            semi_points = [p for p in all_landmarks if f'Forearm_Semi{i}' in p['name']]
            if len(semi_points) == 2:
                semi_points.sort(key=lambda p: p['y'])
                top_edge_points.append((semi_points[0]['x'], semi_points[0]['y']))
                bottom_edge_points.append((semi_points[1]['x'], semi_points[1]['y']))
        ...
        # Process wrist
        wrist_points = [p for p in all_landmarks if 'Wrist' in p['name'] and p['name'] != 'Wrist_Center']
        if len(wrist_points) == 2:
            wrist_points.sort(key=lambda p: p['y'])
            top_edge_points.append((wrist_points[0]['x'], wrist_points[0]['y']))
            bottom_edge_points.append((wrist_points[1]['x'], wrist_points[1]['y']))
        
        # Draw continuous line connecting all top edge points (shoulder to wrist)
        if len(top_edge_points) > 1:
            top_x = [p[0] for p in top_edge_points]
            top_y = [p[1] for p in top_edge_points]
            ax.plot(top_x, top_y, 'r-', linewidth=2, alpha=0.7, label='Top edge')
        
        # Draw continuous line connecting all bottom edge points (shoulder to wrist)
        if len(bottom_edge_points) > 1:
            bottom_x = [p[0] for p in bottom_edge_points]
            bottom_y = [p[1] for p in bottom_edge_points]
            ax.plot(bottom_x, bottom_y, 'r-', linewidth=2, alpha=0.7, label='Bottom edge')
        
        # Close the contour at shoulder end (connect first top to first bottom)
        if len(top_edge_points) > 0 and len(bottom_edge_points) > 0:
            ax.plot([top_edge_points[0][0], bottom_edge_points[0][0]], 
                   [top_edge_points[0][1], bottom_edge_points[0][1]], 
                   'r-', linewidth=2, alpha=0.7)
        
        # Close the contour at wrist end (connect last top to last bottom)
        if len(top_edge_points) > 0 and len(bottom_edge_points) > 0:
            ax.plot([top_edge_points[-1][0], bottom_edge_points[-1][0]], 
                   [top_edge_points[-1][1], bottom_edge_points[-1][1]], 
                   'r-', linewidth=2, alpha=0.7)
        
        # Draw perpendicular lines connecting corresponding top to bottom points
        for i in range(min(len(top_edge_points), len(bottom_edge_points))):
            ax.plot([top_edge_points[i][0], bottom_edge_points[i][0]], 
                   [top_edge_points[i][1], bottom_edge_points[i][1]], 
                   'gray', linewidth=1, alpha=0.3, linestyle='--')
        
        # Draw all landmark points with their ID numbers
        for idx, row in df.iterrows():
            ...
            if row['Type'] == 'Anatomical' and 'Center' in row['Name']:
                ...
                # Add landmark number closer to point
                ax.text(x, y-8, str(landmark_id), fontsize=9, ha='center', fontweight='bold',
                ...
                # Add landmark number closer to point
                ax.text(x, y-6, str(landmark_id), fontsize=7, ha='center', fontweight='bold',
                       bbox=dict(boxstyle='round,pad=0.2', facecolor='yellow', edgecolor='black', alpha=0.85))
